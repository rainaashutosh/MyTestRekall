...
class WindowsRSDSDetector(DetectionMethod):
    ...
    def _test_rsds(self, rsds):
        if (rsds.Signature.is_valid() and
            ...
            known_bad_guid='00625D7D36754CBEBA4533BA9A0F3FE22'
            known_bad_filename='ntkrnlmp.pdb'

            if rsds.GUID_AGE==known_bad_guid and rsds.Filename==known_bad_filename:
                self.session.logging.info("Rekall has picked up wrong GUID. Applying Attivo patch")
                guid='684DA42A30CC450F81C535B4D18944B12'
                pdb_name='ntkrpamp.pdb'
                self.session.logging.info("Supplying %s with GUID %s",pdb_name,guid)
            ...
            if profile:
                self.session.logging.info(
                    "Detected %s with GUID %s", pdb_name,guid)
