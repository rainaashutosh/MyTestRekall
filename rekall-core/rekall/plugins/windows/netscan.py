...
class WinNetscan(tcpip_vtypes.TcpipPluginMixin,
    ...
    table_header = [
        dict(name="offset", style="address",hidden="true"),
        dict(name="protocol", width=8),
        dict(name="local_addr", width=20),
        dict(name="remote_addr", width=30),
        dict(name="state", width=12),
        dict(name="pid", width=5, align="r"),
        dict(name="processname", width=14),
    dict(name="username", width=20),
        dict(name="created",hidden="true")
    ]
        ...
        for run in self.generate_memory_ranges():
            ...
            for (net_object, proto, laddr, lport, raddr, rport,
                ...
                owner = net_object.Owner.dereference_as(
                    vm=self.kernel_address_space, profile=self.session.profile)
                #owner object is if type _EPROCESS object 
                #try to get username of process and if any error occurs output it
                proc_user="-"
                try:
                    EnvironmentBuffer=owner.Peb.ProcessParameters.Environment
                    #EnvironmentVariables=EnvironmentBuffer.__unicode__().split('\n0x')
                    for EnvLine in EnvironmentBuffer:
                        if EnvLine.startswith('USERNAME'):
                            #extract username using regex search
                            regx=r'USERNAME\=(.*)'
                            searchuser=re.search(regx,str(EnvLine),re.I)
                            if searchuser:
                                proc_user=searchuser.group(1)
                            else:
                                proc_user="Error"
                ...
                if str(laddr)!=str(raddr):
                    yield (net_object.obj_offset, proto, lendpoint,rendpoint, state,owner.UniqueProcessId,owner.ImageFileName,proc_user,net_object.CreateTime)
